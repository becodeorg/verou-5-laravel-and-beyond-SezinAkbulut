...
@section('content')
    ...
    <div class="container mt-3">
        ...
        <form action="{{ route('update', ['id' => $product->id]) }}" method="post" enctype="multipart/form-data" class="max-w-md mx-auto border border-gray-300 p-6 rounded-md">
            @csrf
            @method('PUT')
                ...
                <div class="w-1/2">
                    @if($product->photo)
                        <div class="text-center">
                            ...
                            <img src="{{ asset('storage/' . $product->photo) }}" class="card-img-top custom-thumbnail" alt="{{ $product->title }}">
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="flex justify-center mb-6">
                <div class="w-1/2">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-input border w-full p-2" name="title" id="title" value="{{ old('title', $product->title) }}" required>
                    </div>

                    <div class="mb-4">
                        <label for="price" class="block text-gray-700 text-sm font-bold mb-2">Price:</label>
                        <textarea class="form-input border w-full p-2" name="price" id="price" required>{{ old('description', $product->price) }}</textarea>
                    </div>
